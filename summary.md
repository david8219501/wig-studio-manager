...
# 4 חלקים: ביטול-בדיקה + שינוי שם + שיפור InfoTooltip + ניקוי טקסטים

## חלק א': וידוא שהוראה קודמת (התאמת "רווח נקי" לדשבורד) לא בוצעה ✅ נבדק - לא נדרשה פעולה

הוראה קודמת ביקשה לשנות את `row.profit` ב-`Reports.tsx` לחשב
`calculateOrderProfit`+`isActiveOrder`+ הוצאות מסוננות (כמו
`monthlyOperationalProfit` בדשבורד), אבל המשתמש עצר את הביצוע
באמצע **שלב הבדיקה** (לפני כל שינוי קוד) והחליט בסוף שלא לבצע את
זה - שני המדדים נשארים בכוונה עם נוסחאות שונות.

**נבדק ב-grep:** `row.profit` עדיין `income - exp` הפשוט (שורה 175),
**אין** שום `isActiveOrder`/`isInventoryExpenseCategory` ב-
`Reports.tsx` בכלל. ההוראה ההיא באמת לא בוצעה מעולם - המצב כבר
תקין, לא בוצע שינוי.

## חלק ב': שינוי שם "רווח נקי" → "מאזן חודשי" ✅ הושלמה

שינוי תווית בלבד בעמודת הטבלה "דו"ח מכירות מפורט" - זה היה המקום
היחיד באתר שהשתמש בטקסט הזה (`grep -rn "רווח נקי" src/` החזיר תוצאה
אחת בלבד). "רווח משוער" בכרטיס ה-KPI העליון (Reports.tsx) - טקסט
נפרד, לא נגע בו כמבוקש.

**קבצים:** `src/pages/Reports/Reports.tsx`.
...
## חלק ד': ביקורת טקסטים - הסרת שמות שדות מ-InfoTooltip ✅ הושלמה

נסרקו **כל 8** מקומות ה-`InfoTooltip` באתר (`grep -rn "InfoTooltip
text="` + `tooltip:` הנפרד ב-`Reports.tsx`). **6 מתוך 8 דלפו** שם
שדה טכני (`totalPrice`/`paidAmount`) - כולם תוקנו לשפה עסקית פשוטה.
2 הנותרים (Sales.tsx "רווח בפועל", Dashboard.tsx "רווח החודש") כבר
היו בשפה עסקית תקינה - לא שונו.

| קובץ | מיקום | היה (דלף) | הפך אל |
|------|-------|-----------|--------|
| `Sales.tsx` | "יתרת חובות פתוחים" | "סכום totalPrice פחות paidAmount על כל ההזמנות המוצגות כרגע..." | "סכום ההפרש בין המחיר הכולל למה שכבר שולם, על כל ההזמנות המוצגות כרגע..." |
| `Dashboard.tsx` | "חובות פתוחים" (KPI) | "סכום totalPrice פחות paidAmount על כל ההזמנות עם יתרה לתשלום..." | "סכום ההפרש בין מה שגבית למחיר הכולל, על כל ההזמנות שיש להן יתרה לתשלום..." |
| `Dashboard.tsx` | "התפלגות סוגי עבודה" | "אחוז מתוך סך ההכנסה הגולמית (totalPrice) של כל סוגי העבודה..." | "אחוז מתוך סך ההכנסה הגולמית של כל סוגי העבודה..." |
| `Reports.tsx` | "רווח משוער" (KPI שנתי) | "הכנסה גולמית שנתית (totalPrice) פחות כל ההוצאות..." | "הכנסה גולמית שנתית (סך מחירי ההזמנות) פחות כל ההוצאות..." |
| `Reports.tsx` | "מאזן חודשי" (טבלה, לשעבר "רווח נקי") | "הכנסה גולמית של החודש (סכום totalPrice של ההזמנות)..." | "הכנסה גולמית של החודש (סך מחירי ההזמנות)..." |
| `Reports.tsx` | "חובות פתוחים לפי ותק" | "totalPrice פחות paidAmount - כאן, בשונה מ'חובות פתוחים'..." | "ההפרש בין המחיר הכולל למה שכבר שולם - כאן, בשונה מ'חובות פתוחים'..." |

כלל האצבע שהופעל: כל הסבר שמזכיר שם שדה/משתנה (camelCase מעורב
בעברית) נחשב "דולף" ונכתב מחדש - התוכן המהותי (מה מוחרג/כולל, השוואה
לדפים אחרים) נשמר במדויק, רק הניסוח הוחלף לעברית עסקית.
...
וידוא שהוראה קודמת לא בוצעה (row.profit נשאר income-exp, כמתוכנן)
→ שינוי שם עמודה ("רווח נקי" → "מאזן חודשי") → שדרוג עיצובי ל-
InfoTooltip (אייקון SVG אמיתי + בועית בהירה) → ניקוי 6 טקסטים
שדלפו שמות שדה טכניים לשפה עסקית פשוטה. כל חלק עם build+lint נפרד,
commit+push נפרד.
